# 大衍决 App — 开发任务清单

> 最后更新：2026-05-31
>
> **v1.0 定位**：Windows 桌面端优先。专注"输入 → 炼化 → 生成笔记"，修为面板展示列表 + 统计。移动端、macOS/Linux 桌面端延后至 v2。

---

## 一、管道打通（让炼化流程真正跑起来）

### 1.0 用户配置 Python 路径
- [x] 配置页签中增加"Python 路径"字段（类似 `FW_PYTHON` 环境变量）
- [x] 用户可手动指定路径（如安装了 faster-whisper 的 venv Python）
- [x] 留空 = 自动探测系统 Python（PATH 扫描）
- [x] Rust `deps.rs` 读取配置中的 Python 路径，校验版本 >= 3.9
- [x] 探测失败时在配置页签显示警告

### 1.1 接入 Python 脚本
- [x] 将 `D:/Project/MyClaude/myriad-mind/scripts/` 复制到 `scripts/`（上游无独立 repo，用复制+同步说明）
- [x] 验证 6 个脚本可被 Rust `Command::new` 调用
- [x] 实现 `install_faster_whisper.py` 的 venv 自动创建逻辑

### 1.2 前端对接 Rust 后端
- [x] `api.ts` 封装所有 Tauri invoke() 调用（类型安全 + 浏览器降级）
- [x] App.tsx 启动时从 Rust 读取配置，首启自动跳转配置页
- [x] 旧版流式事件监听封装
- [ ] 切换为 `mind-stream-delta` / `mind-stream-status` / `mind-stream-error`
- [x] 进度条对接真实的 `PipelineProgress` 事件

### 1.3 管线编排实现
- [x] Rust `pipeline.rs` 实现 `execute_pipeline()` 主函数
- [x] 串联步骤：识别 → 依赖检查 → 下载 → 音频提取 → ASR → 关键帧 → AI 笔记 → 清理 → 完成
- [x] 每个步骤通过 `pipeline-progress` event 推送到前端
- [x] 缺失工具时优雅跳过（不崩溃）

### 1.4 系统依赖检测
- [x] Rust `deps.rs` 探测 Python / FFmpeg / yt-dlp / CUDA
- [x] 前端 `DepsPanel` 组件展示检测结果（绿/黄状态徽章）

---

## 二、安全与配置

### 2.1 OS 密钥链
- [x] Windows：CredReadW/CredWriteW 读写 Credential Manager
- [x] 替换 config.rs 中的 stub 实现
- [x] 密钥链条目命名规范：`myriad-mind/{service}`
- [x] ~~macOS Keychain~~ → v2
- [x] ~~Linux libsecret~~ → v2

### 2.2 配置页签（与炼化/修为平级）
- [x] 将配置从 Modal 弹窗改为独立顶栏页签（⚙️ 配置），与 📥 炼化 / 📊 修为 平级
- [x] 首次启动检测 — 无配置标记时默认显示配置页签
- [x] 已配置时默认显示炼化页签
- [x] 配置变更保存到 localStorage（后续对接 Rust config.json）
- [x] 敏感字段（API Key）仅显示脱敏形式，编辑时写入 OS 密钥链

### 2.3 配置存储层
- [x] Windows：配置读写到 `%APPDATA%/myriad-mind/config.json`
- [x] 原子写入（tmp + rename）
- [x] `is_first_launch()` 首启检测
- [ ] 版本号 + 迁移逻辑（`version: 1` → 后续升级）
- [x] ~~移动端配置存储~~ → v2

### 2.4 API Key 管理
- [ ] DeepSeek API Key — 从密钥链读取，前端只显示脱敏形式
- [ ] Claude API Key — v2 可选兼容/备用，不作为 v1 必填项
- [x] AI Douyin API Key — 同上
- [x] TikHub Token — 同上
- [x] 火山引擎 VC Token — 同上

### 2.5 AI / MindEngine 接入
- [ ] `DeepSeekClient`：OpenAI-compatible chat completions + stream
- [ ] `MindEngine`：统一 Prompt、Hook、模型调用、事件推送
- [ ] `TaskProfile`：由 `estimator.ts` 输出，供 `ai/routing.ts` 选择 Pro / Flash
- [ ] DeepSeek V4 Pro：长上下文笔记生成，优先利用 1M context
- [ ] DeepSeek V4 Flash：标题、分类、轻量摘要等快速任务

---

## 三、桌面端功能完善

### 3.1 视频处理管线
- [ ] B 站视频：AI Douyin 解析 → 下载 → 提取音频 → ASR → 笔记
- [ ] YouTube：yt-dlp 优先字幕 → 无字幕则下载 + ASR
- [ ] 抖音/小红书：AI Douyin 解析 → 下载 → ASR → 笔记

### 3.2 文章处理管线
- [ ] WebFetch 抓取文章内容
- [ ] 平台反爬降级（知乎/公众号提示用户粘贴 HTML）
- [ ] MindEngine 调用 DeepSeek V4 Pro 生成结构化学习笔记

### 3.3 本地文件处理
- [ ] 本地视频：提取音频 → ASR → 关键帧 → 笔记
- [ ] 本地音频：ASR → 笔记
- [ ] 本地文档：直接读 → DeepSeek 分析 → 笔记
- [ ] 本地目录：扫描 → 逐个处理 → 合并笔记

### 3.4 代码项目分析
- [ ] GitHub URL → git clone → 扫描结构 → DeepSeek 分析 → 架构笔记
- [ ] 本地代码目录同理
- [ ] 灵力评估（按文件数分四级，>300 文件必须确认）

### 3.5 笔记列表展示（修为面板中）
- [ ] 扫描 `note_dir` 下所有 .md，解析 front matter 获取元信息
- [ ] 在修为面板 Dashboard 中展示笔记列表（标题/日期/类型/难度/阅读时长）
- [ ] 点击笔记 → 用系统默认 .md 编辑器打开（`open` / `xdg-open`）
- [ ] ~~SQLite FTS5 全文搜索~~ → v2
- [ ] ~~内嵌 Mermaid 图表渲染~~ → v2
- [ ] ~~截图资产预览~~ → v2
- [ ] ~~内嵌笔记阅读器~~ → 砍掉，用系统编辑器

### 3.6 修为面板
- [ ] 扫描笔记目录统计（已有 `computeStats`）
- [ ] 境界进度 + 成就判定
- [ ] 连续学习天数计算
- [ ] 知识图谱（标签关系图）
- [ ] 面板持久化到 `修为面板.md`

---

## 四、移动端功能完善 [→ v2 延后]

> ⚠️ v1.0 仅 Windows 桌面端。移动端全部延后至 v2。

### 4.1 文章处理 [v2]
- [ ] fetch URL → 提取正文 → DeepSeek / 兼容模型 → 写 .md

### 4.2 笔记列表展示 [v2]
- [ ] 扫描笔记目录，展示列表

### 4.3 修为面板 [v2]
- [ ] 与桌面端共享 `@myriad-mind/core`

---

## 五、构建与分发

### 5.1 桌面端打包
- [ ] Windows：`.msi` 安装包 + `.exe` 便携版
- [ ] 自动更新：`tauri-plugin-updater` + GitHub Releases
- [ ] ~~macOS (.dmg)~~ → v2
- [ ] ~~Linux (.AppImage/.deb)~~ → v2

### 5.2 移动端打包 [→ v2]
- [ ] Android / iOS / 鸿蒙 — 全部延后

### 5.3 CI/CD
- [ ] GitHub Actions：push main → lint + test + build
- [ ] release tag → 全平台构建 → GitHub Releases 发布

---

## 六、品质与文档

### 6.1 测试
- [ ] `packages/core` 单元测试（vitest）
- [ ] `packages/ui` 组件测试
- [ ] Rust `error.rs` / `commands/` 单元测试
- [ ] E2E 测试（Tauri + Playwright？）

### 6.2 文档
- [ ] README 安装说明（各平台依赖要求）
- [ ] README 免责声明（已分析法律风险）
- [ ] 用户文档：如何申请各 API Key
- [ ] 贡献指南 CONTRIBUTING.md

### 6.3 法律合规
- [ ] README 顶部免责声明 ✅（已规划）
- [ ] MIT 许可证保留原作者版权
- [ ] 不内置任何 API Key / Token
- [ ] 不提供分享/发布功能

---

## 进度总览

| 阶段 | 状态 | 完成日期 |
|------|------|----------|
| 项目脚手架 | ✅ 完成 | 2026-05-31 |
| `@myriad-mind/core` 核心逻辑 | ✅ 完成 | 2026-05-31 |
| `@myriad-mind/ui` 组件库 | ✅ 完成 | 2026-05-31 |
| Desktop 前端页面 | ✅ 完成 | 2026-05-31 |
| Desktop Rust 后端 | ✅ 完成 | 2026-05-31 |
| CC Switch 暗色风格重构 | ✅ 完成 | 2026-05-31 |
| 一、管道打通 | ✅ 完成 | 2026-05-31 |
| 二、安全与配置 | ✅ 完成 | 2026-05-31 |
| 三、桌面端功能完善 | ⏳ 待开始 | — |
| 四、移动端功能完善 | 🔒 v2 | — |
| 五、构建与分发 | 🔒 v2 | — |
| 六、品质与文档 | ⏳ 待开始 | — |

---

## 当前里程碑

```
Windows v1.0 ─── 一、管道打通 ✅ ─── 二、安全配置 ✅ ─── 三、功能完善 ─── ✗  ← 你在这里
                                                                             │
v2 ─── 移动端 ─── macOS/Linux ─── 构建分发 ─── 测试文档 ──────────────── ✗
```
