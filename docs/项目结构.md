...
└── commands/          # Tauri IPC 命令模块
    ...
    ├── config.rs      #   配置读写（config.json，含 API Key）
    │                     #   - ~/.myriad-mind-app/config.json（原子写入）
    │                     #   - get/set_config_value 单字段读写 + write_config secret 保护
    │                     #   - 首启检测、配置重置
    ├── deps.rs        #   系统依赖检测
...
| 文件 | 位置 | 内容 |
|------|------|------|
| `config.json` | `~/.myriad-mind-app/` | 全部配置含 API Key（功能开关、ASR 参数、输出路径、模型参数、deepseek_api_key、ai_douyin_api_key 等），明文，不入 git |
