...
> **迁移说明（2026-06-24）**：`list_ai_douyin_tasks.py` 与 `extract_keyframes.py` 已迁移为 Rust 直连实现（`commands/ai_douyin.rs` + `commands/media.rs`），不再作为 Python 脚本存在。剩余 4 个脚本仅用于 whisper（无 Rust 等价）和 yt-dlp（Python 项目）硬依赖。详见 [`设计文档/工程化/Python到Rust迁移计划.md`](设计文档/工程化/Python到Rust迁移计划.md)。
...
| 原脚本 | Rust 替代 | 方案 |
|--------|----------|------|
| ~~`list_ai_douyin_tasks.py`~~ | `commands/ai_douyin.rs` | `reqwest::Client` 直调 AI Douyin API |
| ~~`extract_keyframes.py`~~ | `commands/media.rs` | `std::process::Command` 直调 FFmpeg |

Rust 封装规则（`commands/python.rs`）— 仅适用于保留的 4 个 Python 脚本：
