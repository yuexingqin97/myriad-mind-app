...
>
> 🧭 **AI 处理模式已演进为 Agent**（2026-06-22 代码落地，`cargo check` 0 错误，**待真实 DeepSeek API 端到端验证**）：`execute_pipeline` 现调度目标驱动 Agent（AI 在六阶段骨架内自主选工具，DeepSeek tool use 非流式 loop），取代原 10 步管线。本文 §五/§六/§七 仍描述旧管线实现（作历史参考，待改写）；agent 实现见 `src/agent/` + `src/commands/tools/`，设计见 [`Agent架构设计.md`](设计文档/AI与模型/Agent架构设计.md) / [`Agent 开发计划.md`](设计文档/AI与模型/Agent%20开发计划.md)（含执行状态与验证缺口）。
...
└── commands/          # Tauri IPC 命令模块
    ...
    ├── python.rs      #   Python 脚本调度（剩余 4 个 whisper/yt-dlp 脚本的类型化封装）
    │                     #   - scripts_dir(): 智能 scripts/ 路径解析（dev/venv/exe）
    │                     #   - 4 个 #[tauri::command]: transcribe_audio / download_video
    │                     #     / download_youtube_subtitles / install_faster_whisper
    │                     #   - extract_keyframes / list_ai_douyin_tasks 已迁 media.rs / ai_douyin.rs
    ├── ai_douyin.rs   #   AI Douyin 任务查询（Rust 直连 reqwest，原 list_ai_douyin_tasks.py）
    ├── media.rs       #   关键帧抽取（Rust 直调 FFmpeg，原 extract_keyframes.py）
...
4.5  字幕分析            AI 推荐截图时间点
4.7  精准截图            media.rs（Rust 直调 FFmpeg）+ DeepSeek Vision 审查
...
> **演进（2026-06-24）**：原 6 个脚本中，纯中转的 2 个已迁移为 Rust 直连，不再走 Python：
> - `extract_keyframes.py` → `commands/media.rs`（Rust 直调 FFmpeg）
> - `list_ai_douyin_tasks.py` → `commands/ai_douyin.rs`（Rust 直连 reqwest）
>
> 详见 [`Python到Rust迁移计划.md`](设计文档/工程化/Python到Rust迁移计划.md)。本目录仅保留 whisper / yt-dlp 硬依赖脚本。

```
scripts/
├── README.md                          # 脚本清单 + CLI 接口 + 同步说明 + 迁移记录
├── transcribe_faster_whisper.py       # 音频转写 ASR（faster-whisper）
├── download_video_candidates.py       # 视频下载（多 URL 候选；yt-dlp 部分保留）
├── download_youtube_subtitles.py      # YouTube 字幕下载（yt-dlp）
└── install_faster_whisper.py          # faster-whisper venv 自动安装
```

Rust 封装规则（剩余脚本仍走 `commands/python.rs`）：
...
> 前端不直接调用脚本，一律经 Rust 类型化封装。已迁 Rust 直连的 2 项不再经子进程：FFmpeg 由 `media.rs` 直接 `Command::new("ffmpeg")` 调度，AI Douyin 由 `ai_douyin.rs` 直接 `reqwest` 调用。
>
> **下载总超时兜底（2026-06-24）**：`run_python_script` 子进程加 `kill_on_drop(true)`；`download_douyin_video` 用 `tokio::time::timeout(300s)` 包裹，超时杀子进程并返回错误（B 站自动 fallback yt-dlp），避免 urllib socket 级 timeout 在慢速 CDN 上永久卡死。详见 [迁移计划 §七](设计文档/工程化/Python到Rust迁移计划.md)。
...
apps/desktop/src/
  ...
  └── 调用 api.ts ──Tauri IPC──▶ src-tauri/src/commands/
                                      ...
                                      ├── deps.rs         （依赖检测）
                                      ├── python.rs       （剩余 4 脚本调度）──▶ scripts/*.py
                                      ├── media.rs        （关键帧抽取，Rust 直调 FFmpeg）
                                      ├── ai_douyin.rs    （AI Douyin 查询，Rust 直连 reqwest）
