...
└── commands/          # Tauri IPC 命令模块
    ...
    ├── ai_douyin.rs #  AI Douyin 任务查询（reqwest 直连，原 list_ai_douyin_tasks.py）
    ├── python.rs      #   Python 脚本调度（4 个脚本的类型化封装）
    │                     #   - scripts_dir(): 智能 scripts/ 路径解析（dev/venv/exe）
    │                     #   - 4 个 #[tauri::command]: transcribe_audio
    │                     #     / download_video / download_youtube_subtitles
    │                     #     / install_faster_whisper
    │                     #   - extract_keyframes / list_ai_douyin_tasks 已迁出（见 ai_douyin.rs / pipeline.rs）
...
4.5  字幕分析            AI 推荐截图时间点
4.7  精准截图            原生 FFmpeg 关键帧提取 + DeepSeek Vision 审查
...
> **已迁移为 Rust 实现**（2026-06-24）：`list_ai_douyin_tasks.py`（→ `commands/ai_douyin.rs`，reqwest 直连）与 `extract_keyframes.py`（→ `commands/pipeline.rs`，原生 FFmpeg）。
...
→ 原生 FFmpeg 关键帧提取（pipeline.rs）精准截图
