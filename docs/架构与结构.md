...
4.5  字幕分析            AI 推荐截图时间点
4.7  精准截图            commands/media.rs (FFmpeg) + DeepSeek Vision 审查
...
├── download_video_candidates.py       # 视频下载（多 URL 候选，yt-dlp）
├── download_youtube_subtitles.py      # YouTube 字幕下载（yt-dlp）
└── install_faster_whisper.py          # faster-whisper venv 自动安装

> 注：`extract_keyframes.py` 与 `list_ai_douyin_tasks.py` 已迁移为 Rust 直连（`commands/media.rs`、`commands/ai_douyin.rs`），脚本文件已删除。
```
...
→ commands/media.rs 精准截图（原 extract_keyframes.py 已迁移为 Rust 直连）
