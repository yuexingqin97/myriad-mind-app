...
| 健康项 | 当前问题 | 应改为 |
|--------|----------|--------|
| AI 模型 | 只要有 `keychain` adapter 就显示已连接密钥链 | 调用 `keychain.check("deepseek-api-key")`，存在才显示正常 |
...
| UI 控件 | 当前代码状态 | 目标行为 |
|---------|--------------|----------|
| 打开配置向导 | `onOpenWizard` 为空 | 打开内联 `ConfigWizard`，保留当前配置作为初始值 |
| 选择输出目录 | `onSelectOutputDir` 为空 | 调用 Tauri dialog，选中后写入 `output.note_dir` |
| 打开输出目录 | `onOpenOutputDir` 为空 | 调用 shell open / opener 打开目录 |
...
- `SettingsView` 增加 `wizardOpen` 状态，不再只依赖 `firstLaunch`。
- `SettingsPage.onOpenWizard` 设置 `wizardOpen = true`。
- `ConfigWizard` 支持 `initialStep`，用于从缺项直接进入对应步骤。
- `useConfig` 增加 `setupStatus`，由配置目录、Schema 校验和 DeepSeek Key 共同决定。
- `InputView` 接收 `setupStatus/configHealth`，在未就绪时显示首页引导空态。
- `usePipeline.submit()` 在运行前检查配置，不满足时不调用 `execute_pipeline`。
- `api.detectAllDeps()` 失败时返回结构化错误，不让原始 `invoke` TypeError 直接进入用户文案。
