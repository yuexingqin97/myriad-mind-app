...
Markdown 正文
  └── ## 大衍决心得
        ...
        ├── ### 问答记录：追问模式追加记录（如有）
        ├── ### 元信息：机器可读 YAML 块
        └── ### 调试信息：仅 debug_metadata=true 时输出，放在最末尾
```
...
| 更新时间 | 更新内容 |
|----------|----------|
| 2026-05-31 21:30 | 初次炼化 · 来源: https://www.bilibili.com/video/BVxxx |
| 2026-05-31 22:45 | 重新炼化 · 来源: https://youtube.com/... |
...
- 所有元数据统一归入 `## 大衍决心得` 二级标题下。
- `### 更新记录` 使用表格格式：`| 更新时间 | 更新内容 |`。
- 同一输入再次炼化：追加新行到更新记录表格，`current_version` 递增。
- `### 元信息` 以 `<!-- MYRIAD_MIND_METADATA_START -->...<!-- END -->` 包裹 YAML。
- `### 调试信息` 只有 `debug_metadata=true` 时输出，**放在最末尾**（元信息之后）。
- App 写入时只追加 `## 大衍决心得` 下的子章节，不修改用户正文。
...
## 大衍决心得
  ...
  ├── ### 问答记录（如有）
  ├── ### 元信息（YAML 块）
  └── ### 调试信息（最末尾，debug_metadata=true 时）
```

理由：

- 所有元数据统一在一个二级标题下，用户一目了然。
- 更新记录用表格，更紧凑直观。
- 调试信息放最末尾，最不干扰阅读。
- 元信息块位置固定，程序从文件末尾向前查找。
