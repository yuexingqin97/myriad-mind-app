...
核心差异：Skill 原型靠 Claude 的推理能力 + 多模态视觉做截图审查/字幕分析，App 已用 DeepSeek Vision 视觉模式替代（见 [DeepSeek视觉-截图审查方案](../AI与模型/DeepSeek视觉-截图审查方案.md)），字幕分析/guided 截图/截图审查/教程检测均已上线；评论区精华、自动翻译、修为面板仍待开发。
