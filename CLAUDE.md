...
独立跨平台 App，将大衍决 Claude Code Skill 改造为桌面 + 移动应用。免费开源（MIT）、本地运行、无服务器。用户丢入视频链接/文章 URL/本地文件，自动炼化为结构化学习笔记。

## 当前状态

📋 **规划阶段** — 架构设计和法律风险评估已完成，尚未开始编码。monorepo 脚手架尚未搭建。
...
**关键版本**: Rust 1.93+, Node.js 24+, TypeScript 5.9+, React 19+, Tauri 2.x, Expo SDK 54+, Python 3.9+ (仅桌面), FFmpeg 8.x (仅桌面)
...
移动端不跑重型工具。主攻：笔记阅读 (WebView + Mermaid)、文章 URL 处理 (fetch → Claude API → 写 .md)、修为面板 (纯计算)、搜索、配置。不可替代桌面端做视频处理。

### 安全模型

- `myriad-mind-config.json` — 非敏感配置 (功能开关/输出路径/模型参数)
- **OS 密钥链** — 所有 API Key/Token (Claude Key, AI Douyin Key, TikHub Token)，永不明文存储
